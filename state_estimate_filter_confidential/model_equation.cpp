...
: ModelEquation(obj)
{
  observationCur_ = obj.observationCur_;
  // こちらは別ポインタで確保. 分散の値だけ持っておけばよいのでポリモフィジックな動作は期待しない.
  matVarianceObservationOri_ = new cv::Mat(1, 1, CV_64F);
  *matVarianceObservationOri_ = observationCur_->MatVariance_.clone();
}
...
ObservationModelEquation::ObservationModelEquation(
  ProbabilityDistribution ^i_noize)
: ModelEquation(i_noize)
{
  observationCur_ = i_noize;
  matVarianceObservationOri_ = new cv::Mat(1, 1, CV_64F);
  *matVarianceObservationOri_ = observationCur_->MatVariance_.clone();

#if 1
  double result = matVarianceObservationOri_->at<double>(0, 0);
#endif
}

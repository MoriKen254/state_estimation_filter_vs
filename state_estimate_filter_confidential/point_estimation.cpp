  ...
  AbstractStateEstimateFilter^ i_stateEstimateFilter)
{
  ...
  List<Particle^>^ estimationCur;
#if 0
  //-- センサが異常でreweightを行った場合→リサンプリング&reweight後の最大値計算
  if (((ParticleFilter^)i_stateEstimateFilter)->stateObserve_ == 1)
    estimationCur = static_cast<ParticleDistribution^>(i_stateEstimateFilter->PosterioriEstimationCur_)->Particles_;
  //-- センサが正常でreweightを行わない場合→リサンプリング前の分布で最大値計算
  else
#endif
    estimationCur = static_cast<ParticleDistribution^>(i_stateEstimateFilter->PriorEstimationCur_)->Particles_;

  int weightMaxIndex = 0;
  double weightMax = 0;

  // act
  for (int i = 0; i < estimationCur->Count; i++)
  {
    // 最大値更新なし
    if (weightMax > estimationCur[i]->Weight_)
      continue;

    // 最大値更新
    weightMax = estimationCur[i]->Weight_;
    weightMaxIndex = i;
  }
#if MY_DEBUG
  double result[1000] = { 0 };
  ...
  return estimationCur[weightMaxIndex]->StateDistribution_->VecMean_;
}
...
{
  // arrange
  //-- 現時刻　リサンプリング前 事前推定パーティクル群(ノイズ有)
  List<Particle^>^ priorCurParticleListBeforeResample
    = static_cast<ParticleDistribution^>(i_stateEstimateFilter->PriorEstimationCur_)->Particles_;
  //-- 現時刻　事前推定パーティクル群(ノイズ無し)
  List<Particle^>^ priorCurParticleListWithoutNoise
    = gcnew List<Particle^>(priorCurParticleListBeforeResample);
  ...
  double posterioriSum = 0;
  //-- 結果出力用に加工。消しても良い。
  //---- 事後確率 -　事後確率最大
  posterioriOfXhatFromYCur_ = new vector<double>(posterioriOfXhatFromYCur.size(), 0);
  for (int indexParticle = 0; indexParticle < posterioriOfXhatFromYCur.size(); indexParticle++)
  {
    (*posterioriOfXhatFromYCur_)[indexParticle] = 
      exp(posterioriOfXhatFromYCur[indexParticle].at<double>(0, 0) - posterioriOfXhatFromYCur[mapMaxIndex].at<double>(0, 0));

    posterioriSum += (*posterioriOfXhatFromYCur_)[indexParticle];
  }
  //---- 正規化
  for (int indexParticle = 0; indexParticle < posterioriOfXhatFromYCur.size(); indexParticle++)
  {
    (*posterioriOfXhatFromYCur_)[indexParticle] = (*posterioriOfXhatFromYCur_)[indexParticle] / posterioriSum;
  }
   
  // MAP推定した状態
  return priorCurParticleListBeforeResample[mapMaxIndex]->StateDistribution_->VecMean_;
}
  ...
  AbstractStateEstimateFilter^ i_stateEstimateFilter)
{
  ...
  for (int i = 0; i < countParticles; i++)
  {
    result[indexResult++] = posterioriOldParticleList[i]->StateDistribution_->VecMean_.at<double>(0, 0);
    result[indexResult++] = priorCurWithoutNoiseParticleList[i]->StateDistribution_->VecMean_.at<double>(0, 0);
  }
#endif

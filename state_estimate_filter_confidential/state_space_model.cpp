...
, List<ObservationModelEquation^>^ i_observationModelEquations
, const cv::Mat i_vecStateTrueInit)
  : processModelEquation_(i_processModelEquation)
  , observationModelEquations_(i_observationModelEquations)
{
  vecStateTrueCur_ = new cv::Mat(i_vecStateTrueInit);
#if 1
  double var = ObservationModelEquations_[0]->observationCur_->MatVariance_.at<double>(0, 0);
  double varOri = ObservationModelEquations_[0]->matVarianceObservationOri_->at<double>(0, 0);
#endif
#if 0
  cv::Mat vecObservationMeanInit = cv::Mat::zeros(1, 1, CV_64F);

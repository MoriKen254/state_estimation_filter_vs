...
{
  cv::Mat vecInputCur = (cv::Mat_<double>(1, 1) << i_vecInputCur);
  this->Predict(vecInputCur);  // 一段先予測
  this->Filter(); // フィルタリング
}
...
// cv::Matを引数に取ると外部参照として使えないので，ラップする．
void AbstractStateEstimateFilter::UpdateToNextTime(
  const double i_vecInput)
{
  cv::Mat vecInputCur = (cv::Mat_<double>(1, 1) << i_vecInput);

  // 現時刻事後推定値　→　一時刻前事後推定値にシフト
  PosterioriEstimationOld_ = gcnew ProbabilityDistribution(PosterioriEstimationCur_);

  // 真値更新(状態空間モデル内の真値を更新する)
  StateSpaceModel_->UpdateInnerTrueValueWithNoise(vecInputCur, TimeCur_);
  ...
  EstimationResult^ i_estimationResult)
{
  ...
  double priorEstimation = priorEstimationCur_->VecMeanList_[0];// VecMean_.at<double>(0, 0);
  ...
  //-- 事前推定値
  i_estimationResult->priorEstimations->Add(priorEstimation);
  //-- 真値

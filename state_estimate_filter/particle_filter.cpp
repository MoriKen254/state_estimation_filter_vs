...
void ParticleFilter::Filter()
{
  ...
  for (int indexSensor = 0; indexSensor < countSensor; indexSensor++)
  {
    ...
    // 通常尤度で足し算
    for (int indexParticle = 0; indexParticle < countParticle; indexParticle++)
      (*logLikelihoodsPriorStateToObserve_)[indexParticle] += exp(logWeightSingle[indexParticle]);
#if 0
    // CalcLikelifood(); 
    // CalcWeight();
#endif
  ...
  // 対数尤度に戻す
  for (int indexParticle = 0; indexParticle < countParticle; indexParticle++)
    (*logLikelihoodsPriorStateToObserve_)[indexParticle] = log((*logLikelihoodsPriorStateToObserve_)[indexParticle]);
  ...
  AbstractPointEstimation^ i_pointEstimation)
{
  ...
  PosterioriEstimationCur_->VecMean_ = i_pointEstimation->EstimateFromParticles(this);
}
